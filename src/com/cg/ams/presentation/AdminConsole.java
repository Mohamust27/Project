...
public class AdminConsole {
	...
	public void start(String userName) throws AMSException {
		while (true) {
			...
			System.out.println("5. Update asset if required");
			...
			case 1:
				...
				System.out.println("asset inserted with assetId "
						+ assetbean.getAssetId());

				break;
			case 2:
				List<AssetAllocationBean> list = service.viewRequest();
				for (AssetAllocationBean assetAllocationBean : list) {
					System.out.println(assetAllocationBean);
				}
			...
			case 3:
				...
				if (quantity1 > 0) {
					...
					System.out.println("Request to allocate asset: "
							+ assetallocation.getAssetName()
							+ " to employee Id: " + assetallocation.getEmpNo());
				} else {
					System.err.println("Wrong asset Name");
					System.exit(0);
				}
				int Quantity = service.viewQuantity(assetallocation
						.getAssetName());
				if (Quantity > 0) {
					System.out.println(assetallocation.getAssetName()
							+ "is approved to" + assetallocation.getEmpNo());
					service.decrementQuantity(assetallocation.getAssetName());
					service.deleteRequest(assetallocation.getEmpNo());
				} else {
					int delete = service.deleteAsset(assetallocation
							.getAssetName());
					System.err
							.println("Wrong assent name enetered or asset is not available");
			...
			case 4:
				...
				System.out.println("Request to deallocate asset: "
						+ assetallocation.getAssetName() + " to employee Id: "
						+ assetallocation.getEmpNo());

				int Quantity1 = service.viewQuantity(assetallocation
						.getAssetName());
				if (Quantity1 > 0) {
					System.out.println(assetallocation.getAssetName()
							+ "is removed from " + assetallocation.getEmpNo());
			...
			case 5:
				...
				int quantitys = scanner.nextInt();
				assetbean.setAssetName(assetName1);
				assetbean.setQuantity(quantitys);
				service.updateAsset(assetbean);
